...
5. **Key administrators**: bỏ qua -> Next
6. **Key users**: bỏ qua -> Next
...
<img width="1087" height="320" alt="516210080-83274383-edf5-4f39-925a-75abdcbff03a" src="https://github.com/user-attachments/assets/350e5d1b-5dba-416c-9e5f-1a6ed79a00bf" />
...
!!! note "Signature & Hash – Giải thích chi tiết"
    ...
    - **"AI gửi request này?" (Authenticate)**  
        Đảm bảo user là người hợp lệ, có quyền gửi lệnh đến AWS.  
        → AWS xác thực dựa trên Signature + Access Key ID.  
    - **"Request này có bị thay đổi không?" (Integrity)**  
        Nếu request bị chỉnh sửa (vd: thay URL, body), chữ ký (Signature) sẽ không hợp lệ.
    ...
    **Lệnh:**  
    ```bash
    aws s3 cp secret-file.txt s3://bucket/encrypted-file.txt --profile admin
    ```
    ...
    - **Headers:** `Host`, `x-amz-date`, `x-amz-security-token`,...  
    - **Body:** Nội dung file (nếu có).
    ...
    ### **BƯỚC 2: TẠO "CANONICAL REQUEST"**
    Canonical Request là "bản chuẩn hóa" của request, dùng để **đảm bảo tính toàn vẹn lệnh**.  
    ...
    **Ví dụ Canonical Request:**  
    ...
    ### **BƯỚC 3: TẠO "STRING TO SIGN"**
    Bao gồm:
    1. Thuật toán: `AWS4-HMAC-SHA256`
    2. Thời gian request: `20251120T151000Z`
    3. Region + Service: `us-east-1/s3/aws4_request`
    4. Hash của Canonical Request
    ...
    **Ví dụ String to Sign:**
    ...
    Dựa trên Secret Access Key với nhiều lớp HMAC

    ```
    DateKey          = HMAC("AWS4" + SecretKey, "20251120")
    DateRegionKey    = HMAC(DateKey, "us-east-1")
    ServiceKey       = HMAC(DateRegionKey, "s3")
    SigningKey       = HMAC(ServiceKey, "aws4_request")
    Signature        = HMAC(SigningKey, StringToSign)
    ```

    **Ví dụ Signature cuối:**
    ```
    ...
    ### **BƯỚC 5: GỬI REQUEST VỚI SIGNATURE**
    ```http
    Authorization: AWS4-HMAC-SHA256 
    Credential=AKIA.../20251120/us-east-1/s3/aws4_request,
    ...
    - So sánh signature tính lại với signature client gửi lên
    → Không khớp sẽ lỗi `SignatureDoesNotMatch`

    ---

    ## **HASH – ĐẢM BẢO INTEGRITY NỘI DUNG FILE**

    ### Khi upload file lên S3:
    1. CLI tính MD5 → gửi kèm
    2. S3 kiểm tra lại → nếu sai báo lỗi
    3. ETag = MD5 của file

    ---

    ### Khi download file từ S3:
    - CLI tính lại MD5 và so sánh với ETag  
    → Sai = lỗi truyền tải
