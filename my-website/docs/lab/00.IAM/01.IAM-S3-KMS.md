...
??? note "Signature & Hash – Giải thích chi tiết"
    ...
    - **"Request này có bị thay đổi không?" (Integrity)** Nếu request bị chỉnh sửa (vd: thay URL, body), chữ ký (Signature) sẽ không hợp lệ.
    ...
    **Ví dụ Canonical Request:** 
    ...
    1. **Thuật toán sử dụng (Signature Version 4):**
       → `AWS4-HMAC-SHA256`
    ...
    #### **Quy trình:**
    1. **Tạo Signing Key từ Secret Key:**
       ```
    ...
    2. **Ký String to Sign:**
       ```
       Signature = HMAC(SigningKey, String to Sign)
       ```
    ...
    **Kết quả cuối cùng (signature):**
    ...
    #### **Quy trình:**
    1. AWS CLI tính **Hash MD5** của file trước khi upload. 
    2. Gửi Hash MD5 kèm nội dung file.
       - Ví dụ (Header HTTP): `Content-MD5: abc123...`.
    3. **AWS S3 tự tính lại hash** của nội dung sau khi nhận.
    4. So sánh MD5:
       - **Khớp:** File nguyên vẹn → Chấp nhận lưu.  
       - **Không khớp:** Báo lỗi: `Invalid Content-MD5`.  
    5. Tạo ETag = Hash MD5 → Lưu kèm file.  

    ---

    ### **2. KHI DOWNLOAD FILE TỪ S3**
    #### **Quy trình:** 1. S3 trả về file cùng ETag (Hash MD5).  
    2. AWS CLI tính lại Hash MD5 của file tải về.  
    3. **So sánh Hash:**
